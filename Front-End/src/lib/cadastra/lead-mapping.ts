...
//   1. Empresa cola a URL `/api/empresas/{empresaId}/cloudia/webhook` no painel do Kommo
...
import type { KommoFieldTarget } from '@/lib/cadastra/api'
...
// ----------------------------------------------------------------------------
// Mapeamento canônico: campos do payload do webhook Kommo
// ----------------------------------------------------------------------------
// Este é o mapeamento REAL que o backend usa. As colunas das planilhas são
// derivadas destes campos.
export interface WebhookField {
  // Caminho dentro do payload do Kommo (ex.: "data.name", "data.tags[].name").
  path: string
...
export const CLOUDIA_WEBHOOK_FIELDS: WebhookField[] = [
  ...
  { path: 'data.origin',                     cadastroTarget: 'origem',               netEntity: 'Lead',      netField: 'Source',              notes: 'Casado contra LEAD_ORIGENS — fallback "Sem origem".' },
...
export const LEAD_EVENT_TYPES: { type: import('@/lib/cadastra/api').LeadEventType; label: string; description: string; recommended: boolean }[] = [
  { type: 'CUSTOMER_CREATED',           label: 'Lead criado',          description: 'Novo lead aparece no Kommo. Vai p/ inbox como pendente.',     recommended: true },
...
export const LEAD_SAMPLES: SpreadsheetField[] = [
  { column: 'ID',                          cadastroTarget: null,                    netEntity: 'Contact', netField: 'ExternalId',          notes: 'GUID curto do Kommo. Vai para Lead.ExternalId / Contact.ImportBatchId.' },
  ...
  { column: 'Origem Cadastro',             cadastroTarget: 'origem',                netEntity: 'Lead',    netField: 'Source',              notes: 'Lista fixa (ver LEAD_ORIGENS abaixo).' },
  ...
  { column: 'Motivo para Não Agendamento', cadastroTarget: 'motivoNaoAgendamento',  netEntity: 'Lead',    netField: 'Observations',        notes: 'Lista fixa (ver LEAD_MOTIVOS_NAO_AGENDAMENTO abaixo).' },
...
// ---- Lista canônica de origens (como está no Kommo) ----
...
export type CloudiaOrigem = (typeof LEAD_ORIGENS)[number]

// Origens que automaticamente classificam o lead como "Resgate"
export const RESGATE_ORIGENS: CloudiaOrigem[] = [
  'Resgate: Disparo em massa',
  'Resgate: Disparo de agendamento',
  'Resgate: Ligação',
  'Resgate: Mensagem',
...
// ---- Motivos para não agendamento (lista fixa do Kommo) ----
export const LEAD_MOTIVOS_NAO_AGENDAMENTO = [
...
// ---- Helper: dado o payload do webhook Kommo, retorna o que preencher no lead-form ----
...
import type { SourceWebhookEventDto } from '@/lib/cadastra/api'
...
export function cloudiaWebhookToLeadForm(payload: CloudiaPayloadShape) {
  ...
  const isResgate = RESGATE_ORIGENS.includes(origem as CloudiaOrigem)
...
export function parseCloudiaEvent(event: SourceWebhookEventDto): CloudiaPayloadShape | null {
  try {
