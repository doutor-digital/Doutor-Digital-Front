...
export function LeadsFimDeSemanaCard({
  ...
  return (
    <div
      className={`rounded-2xl border border-slate-200 bg-white border border-slate-200 p-5 ${className}`}
    >
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <h3 className="text-sm font-semibold text-slate-900">Leads do fim de semana</h3>
          <p className="text-xs text-slate-400">
            Sábado e domingo no período · dia comercial 19h→19h
          </p>
        </div>
        <div className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-slate-50 text-slate-300 ring-1 ring-inset ring-slate-200">
      ...
      {loading ? (
        ...
        <>
          <div className="flex items-end gap-5">
            <p className="text-5xl font-bold leading-none tabular-nums text-slate-900">
